...
(function($) {
  var MorseCodeInput = {
    ...
    down: function() {
      ...
      if (MorseCodeInput.input_started === false) {
        ...
        MorseCodeInput.short_gap_timer = window.setTimeout(
          MorseCodeInput.input_ends,
          MorseCodeInput.dot_duration * 3
        );
      }

      MorseCodeInput.down_timer = window.setTimeout(function() {
        $("#morse-code-input-div .circle-big").show();
      }, MorseCodeInput.dot_duration);
      ...
      if (
        ...
        (duration = new Date() - MorseCodeInput.up_date) &&
        duration > MorseCodeInput.dot_duration * 30
      ) {
    ...
    up: function() {
      ...
      if (
        MorseCodeInput.input_started &&
        duration > 5 &&
        duration <= MorseCodeInput.dot_duration
      ) {
        MorseCodeInput.short_gap_test();
      ...
      if (MorseCodeInput.down_timer !== null)
        window.clearTimeout(MorseCodeInput.down_timer);
    ...
    short_gap_test: function() {
      if (
        MorseCodeInput.input_started &&
        MorseCodeInput.short_gap_timer !== null
      ) {
    ...
    input_ends: function() {
      if (MorseCodeInput.input_value !== "") {
        if (MorseCodeInput._code[MorseCodeInput.input_value]) {
          MorseCodeInput.text_value +=
            MorseCodeInput._code[MorseCodeInput.input_value];
          console.log("hi " + MorseCodeInput.text_value);
          $("#morse-code-input-text > div").text(MorseCodeInput.text_value);
        }
      }

      if (MorseCodeInput.short_gap_timer !== null)
        window.clearTimeout(MorseCodeInput.short_gap_timer);
    ...
    activate: function(element) {
      ...
      //console.log("Hi ".MorseCodeInput.active_element.val());

      var html = "";
      html +=
        '<div id="morse-code-input-header"><div>Morse Code Input</div><small>(press any key or tap to start typing)</small><button type="button" class="overlay-close">✖</button></div>';
      html +=
        '<div id="morse-code-input-text"><div>' +
        MorseCodeInput.text_value +
        "</div></div>";
      html +=
        '<div id="morse-code-input-div"><div class="hint">Tap Here to Type</div><div class="circle-small"></div><div class="circle-big"></div><div class="input-dots"></div></div>';
      $("body").prepend(
        "<div id='morse-code-input-overlay'>" + html + "</div>"
      );
    },

    destroy: function() {
      var msg = new SpeechSynthesisUtterance(MorseCodeInput.text_value);
      window.speechSynthesis.speak(msg);
      console.log(MorseCodeInput.text_value);
  ...
  $(document).keyup(function(e) {
    ...
    if (e.keyCode == 27 || e.keyCode == 8 || e.ctrlKey || e.altKey || e.metaKey)
      return;
